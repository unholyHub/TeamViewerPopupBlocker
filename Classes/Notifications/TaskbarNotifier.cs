...
namespace TeamViewerPopupBlocker.Classes.Notifications
{
    ...
    /// <summary>
    /// TaskbarNotifier allows to display MSN style/Skinned instant messaging pop ups
    /// </summary>
    public class TaskbarNotifier : Form
    {
        ...
        private static string familyName = "Arial";

        private static int fontSizeNormal = 13;

        private static int fontSizeHover = 14;

        protected Font normalTitleFont = new Font(familyName, fontSizeNormal, FontStyle.Regular, GraphicsUnit.Pixel);

        protected Font hoverTitleFont = new Font(familyName, fontSizeHover, FontStyle.Bold, GraphicsUnit.Pixel);

        protected Font normalContentFont = new Font(familyName, fontSizeNormal, FontStyle.Regular, GraphicsUnit.Pixel);

        protected Font hoverContentFont = new Font(familyName, fontSizeHover, FontStyle.Regular, GraphicsUnit.Pixel);
        ...
        public void Show(string strTitle, string strContent, int nTimeToShow, int nTimeToStay, int nTimeToHide)
        {
            ...
            switch (taskbarState)
            {
                case TaskbarStates.hidden:
                    ...
                    SetBounds(
                        WorkAreaRectangle.Right - BackgroundBitmap.Width - 17,
                        WorkAreaRectangle.Bottom - 1,
        ...
        protected void DrawCloseButton(Graphics grfx)
        {
            if (CloseBitmap != null)
            {
                ...
                {
                    if (bIsMouseDown) rectSrc = new Rectangle(new Point(CloseBitmapSize.Width * 2, 0), CloseBitmapSize);
                    else rectSrc = new Rectangle(new Point(CloseBitmapSize.Width, 0), CloseBitmapSize);
                }
                else rectSrc = new Rectangle(new Point(0, 0), CloseBitmapSize);
        ...
        protected void DrawText(Graphics grfx)
        {
            if (titleText != null && titleText.Length != 0)
            {
                ...
                if (bIsMouseOverTitle) grfx.DrawString(titleText, hoverTitleFont, new SolidBrush(hoverTitleColor), TitleRectangle, sf);
                else grfx.DrawString(titleText, normalTitleFont, new SolidBrush(normalTitleColor), TitleRectangle, sf);
            }
            ...
            if (contentText != null && contentText.Length != 0)
            {
                ...
                if (bIsMouseOverContent)
                {
                    grfx.DrawString(
                        contentText,
                        hoverContentFont,
                        new SolidBrush(hoverContentColor),
                        ContentRectangle,
                        sf);
                    ...
                    grfx.DrawString(
                        contentText,
                        normalContentFont,
                        new SolidBrush(normalContentColor),
                        ContentRectangle,
                        sf);
            }
        }
        ...
        protected void CalculateMouseRectangles()
        {
            ...
            {
                RealTitleRectangle = new Rectangle(
                    TitleRectangle.Left,
                    TitleRectangle.Top,
            ...
            if (sizefContent.Height > ContentRectangle.Height)
            {
                RealContentRectangle =
                    new Rectangle(
                        (ContentRectangle.Width - (int)sizefContent.Width) / 2 + ContentRectangle.Left,
        ...
        {
            if (bitmap == null) throw new ArgumentNullException("Bitmap", "Bitmap cannot be null!");
            ...
            for (int j = 0; j < height; j++)
            {
                ...
                {
                    if (bitmap.GetPixel(i, j) == transparencyColor) continue;
        ...
        protected override void OnMouseMove(MouseEventArgs mea)
        {
            ...
            {
                if (bIsMouseOverClose || bIsMouseOverTitle || bIsMouseOverContent) bContentModified = true;
